...
# Weather data is averaged for each datetime, defined above, over all five stations
avg_temp = weather_data.groupby("datetime")[["T", "RH"]].mean().reset_index()
print(avg_temp)
...
# Weighted precipitation calculation (same weighting method as for temperature)
avg_temp["weighted_precip"] = avg_temp["RH"] * avg_temp["weight"]

# The weighted temperature average is calculated for each individual month. 
# This is done by dividing the total sum of the weighted temperatures in a month by the total weighted average sum.
# The same is done for precipitation (RH) to get a monthly weighted precipitation value.
monthly_temp = (
    avg_temp.groupby(avg_temp["datetime"].dt.to_period("M"))
...
# NEW SECTION: Plot rainfall comparison with disruptions and disruption minutes
# ---------------------------------------------------------------
